...
for idoc, doc in enumerate(launchpad.fireworks.find(m_query, projection=projection, sort=sortby)):
    print doc
    ...
    if 'prev_vasp_dir' in doc['spec'] and not os.path.exists(doc['spec']['prev_vasp_dir']):
        ...
        if not os.path.exists(launch_dir):
            print '    ---->', '/'.join(block_dir), 'does not exists!'
            break
    if idoc > 25:
        break
